...
lv_obj_t* radioMemoryChannelDropDown;
...
static void GUIBasicRadioConfigDropDownEvent(lv_event_t * e) {
	...
	if(code == LV_EVENT_VALUE_CHANGED) {
		...
		else if(obj == radioMemoryChannelDropDown) {
			uint16_t selected = lv_dropdown_get_selected(obj);
			//Inform Radio Interface of value change
			InterThreadMessageStruct msg = {.id = InterThread_Memory, .data = (uint32_t*)selected, .length = 0 };
			rt_mq_send(&radioInterfaceMessageQueue, (void*)&msg, sizeof(InterThreadMessageStruct));
		}
...
//		}
			...
			//Inform Radio Interface of value change
			InterThreadMessageStruct msg = {.id = InterThread_AFC, .data = (uint32_t*)selected, .length = 0 };
			rt_mq_send(&radioInterfaceMessageQueue, (void*)&msg, sizeof(InterThreadMessageStruct));
		}
...
void GUIBasicRadioConfigAreaInit() {
	...
	//Radio Pre-defined modes
	radioMemoryChannelDropDown = lv_dropdown_create(lv_scr_act());
	lv_dropdown_set_options(radioMemoryChannelDropDown,	"AFSK-1200\n"
														"FSK-9600\n"
														"BPSK-9600\n"
														"FM WB\n"
														"FM NB");
	lv_obj_remove_style_all(radioMemoryChannelDropDown);
	lv_obj_add_style(radioMemoryChannelDropDown, &mainStyle, 0);
	lv_dropdown_set_symbol(radioMemoryChannelDropDown, NULL);
	lv_obj_set_size(radioMemoryChannelDropDown, 90, 30);
	lv_obj_set_pos(radioMemoryChannelDropDown, 60, 105);
	//Style Drop Down list items
	list = lv_dropdown_get_list(radioMemoryChannelDropDown);
	lv_obj_remove_style_all(list);
	lv_obj_add_style(list, &mainStyle, LV_PART_MAIN);
	lv_obj_add_style(list, &mainStyleSelected, LV_PART_SELECTED);
	lv_obj_set_style_text_line_space(list, 6, LV_PART_MAIN);
	//Add event
	lv_obj_add_event_cb(radioMemoryChannelDropDown, GUIBasicRadioConfigDropDownEvent, LV_EVENT_VALUE_CHANGED, NULL);
...
//	lv_obj_add_event_cb(radioModulationDropDown, GUIBasicRadioConfigDropDownEvent, LV_EVENT_VALUE_CHANGED, NULL);
	...
	lv_obj_set_size(radioAGCDropDown, 90, 30);
	lv_obj_set_pos(radioAGCDropDown, 150, 105);
	//Style Drop Down list items
	...
	//Radio AFC Drop Down
	radioAFCDropDown = lv_dropdown_create(lv_scr_act());
	lv_dropdown_set_options(radioAFCDropDown,	"AFC WD\n"
												"AFC MID\n"
												"AFC NW\n"
												"AFC OFF");
	lv_obj_remove_style_all(radioAFCDropDown);
	lv_obj_add_style(radioAFCDropDown, &mainStyle, 0);
	lv_dropdown_set_symbol(radioAFCDropDown, NULL);
	lv_obj_set_size(radioAFCDropDown, 90, 30);
	lv_obj_set_pos(radioAFCDropDown, 240, 105);
	//Style Drop Down list items
	list = lv_dropdown_get_list(radioAFCDropDown);
	lv_obj_remove_style_all(list);
	lv_obj_add_style(list, &mainStyle, LV_PART_MAIN);
	lv_obj_add_style(list, &mainStyleSelected, LV_PART_SELECTED);
	lv_obj_set_style_text_line_space(list, 6, LV_PART_MAIN);
	//Add event
	lv_obj_add_event_cb(radioAFCDropDown, GUIBasicRadioConfigDropDownEvent, LV_EVENT_VALUE_CHANGED, NULL);
...
//	lv_obj_add_event_cb(radioPacketCRCDropDown, GUIBasicRadioConfigDropDownEvent, LV_EVENT_VALUE_CHANGED, NULL);
	...
	lv_obj_set_pos(radioTNCEnableButton, 420, 105);
	//Button Label
	lv_obj_t* label = lv_label_create(radioTNCEnableButton);
